...
To restore Bluetooth, remove the two lines added to `/boot/config.txt`:
...
Then, re-enable the system service initializing the Bluetooth modem:

```bash
sudo systemctl enable hciuart
```

Finally, add the `console=serial0,115200` part back to the beginning of `/boot/cmdline.txt`.
...
Note! For proper operation afterward, it is essential that you remove at least the third jumper from the PROG header. Otherwise, the Raspberry Pi will not be able to turn itself off.
